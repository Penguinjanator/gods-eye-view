...
export function createLocalData({ scene: { viewer }, controls: { styleManager }, allowQaRegistration, defer }) {
...
defer(async () => {
  ...
  if (dataManager.layers.size) throw new Error(`Data layers could not be destroyed: ${[...dataManager.layers.keys()].join(', ')}`);
});
...
if (allowQaRegistration) {
  ...
  defer(() => {
    if (window.__gevQaRegisterLayer === register) delete window.__gevQaRegisterLayer;
    if (window.__gevQaUnregisterLayer === unregister) delete window.__gevQaUnregisterLayer;
  });
}
