...
import {
  installRenderGovernor, getRenderGovernorDiagnostics, governorRequestRender,
  holdContinuousRender, releaseContinuousRender,
} from '../../renderGovernor.js';
import { startLocalChrome } from './startupChrome.js';

/** Attach scene tools, rendering listeners and the standalone debug handle. */
export function createLocalTools({ scene, controls, data, loadingScreen, signal, defer }) {
...
defer(() => { removeTrackingListener(); releaseContinuousRender('tracked-entity'); });
...
defer(() => { if (window.__godsEyeView === debug) delete window.__godsEyeView; });
const voiceCommands = initGevVoiceCommands({ viewer, styleManager, dataManager, sceneDirector, annotations });
defer(() => {
