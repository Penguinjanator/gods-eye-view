...
export function startLocalChrome({ loadingScreen, styleManager, dataManager, signal }) {
  let disposed = false;
  let firstRun;
  let revealTimer;
  let resolveDelay;
  const minimumDelay = new Promise((resolve) => { resolveDelay = resolve; });
  ...
  void Promise.all([styleManager.initialRestorePromise, minimumDelay])
    .catch(() => { /* Restoration reports its own outcome through the controls. */ })
    .then(() => {
  ...
  void keySetup.catch(() => console.error('Provider settings initialization failed'));
  return async () => {
    disposed = true;
