...
test('startup shares one promise and teardown keeps data alive for controls', async () => {
  const { app, events } = fixture({ createTools({ scene, controls, data, defer }) {
  ...
  await stopped;
  assert.deepEqual(events.slice(-4), ['stop:Tools', 'stop:Controls', 'stop:Data', 'stop:Scene']);
...
test('destroy aborts an in-flight constructor and cleans late acquisitions before resolving', async () => {
  ...
  let observedSignal;
  const { app, events } = fixture({ async createScene({ signal, defer }) {
  ...
  let settled = false;
  const stopped = app.destroy().then(() => { settled = true; });
...
test('cleanup errors are reported after all remaining resources are attempted', async () => {
  const { app, events } = fixture({ createTools({ defer }) {
    defer(() => events.push('tools:earlier'));
    defer(() => { throw new Error('dispose failed'); });
    return {};
  } });
  await app.start();
  ...
  assert.deepEqual(events.slice(-4), ['tools:earlier', 'stop:Controls', 'stop:Data', 'stop:Scene']);
  assert.equal(app.getState().status, 'failed');
});

test('subscribers can destroy during startup without allowing the next constructor', async () => {
  const { app, events } = fixture();
  app.subscribe((state) => { if (state.phase === 'controls') void app.destroy(); });
...
test('destroy during ready notification rejects startup and tears everything down', async () => {
  const { app } = fixture();
  app.subscribe((state) => { if (state.status === 'ready') void app.destroy(); });
  await assert.rejects(app.start(), { name: 'AbortError' });
  await app.destroy();
...
test('cleanup registration closes when a constructor settles', async () => {
  let register;
  const { app } = fixture({ createScene({ defer }) { register = defer; return {}; } });
  await app.start();
...
test('the separate viewer export imports without constructing a browser viewer', async () => {
  const { createApplicationViewer } = await import('gods-eye-view/application/viewer');
