...
import { celestrakProxy, rocketLaunchesProxy, launchLibraryRequestHeaders, LL2_CACHE_TTL_MS } from 'gods-eye-view/server/providers/space';
import { celestrakTleUrl, launchLibraryRecentUrl } from 'gods-eye-view/sources/space';
import * as compatibility from '../../server/providers/local.js';

function install(plugin, preview = false) {
  const routes = new Map();
  plugin[preview ? 'configurePreviewServer' : 'configureServer']({ middlewares: { use(route, handler) { routes.set(route, handler); } } });
...
function isolateDisk(t) {
  t.mock.method(fsp, 'readFile', async () => { throw Error('no cache'); });
  t.mock.method(fsp, 'stat', async () => { throw Error('no cache'); });
...
test('portable requests keep fixed origins, encode group data and preserve the 30-day UTC window', () => {
  ...
  assert.deepEqual(Object.fromEntries(url.searchParams), { net__gte: '2026-01-30T12:34:56.000Z', net__lte: '2026-03-01T12:34:56.000Z', limit: '100', mode: 'detailed' });
...
test('compatibility exports retain the same LL2 header helper and TTL', () => {
  assert.equal(compatibility.launchLibraryRequestHeaders, launchLibraryRequestHeaders);
...
test('exported CelesTrak plugin coalesces refreshes, retains stale TLEs, and reads disk in a new instance', async (t) => {
  ...
  release();
  for(const res of await Promise.all([first, second])) assert.equal(res.body, tle);
  assert.equal(calls, 1);
  assert.equal((await request('/api/celestrak', '/stations')).headers['x-tle-cache'], 'HIT');
  now += 6 * 3600_000;
  ...
  t.mock.method(fsp, 'readFile', async () => JSON.stringify({ at: now, body: tle }));
  t.mock.method(globalThis, 'fetch', async () => { throw Error('fresh disk must prevent fetch'); });
...
for(const preview of [false, true]) test(`exported launch plugin preserves optional server auth and cache in ${preview ? 'preview' : 'development'}`, async (t) => {
  isolateDisk(t);
  const prior = process.env.LL2_API_TOKEN;
  t.after(() => { if (prior === undefined) delete process.env.LL2_API_TOKEN; else process.env.LL2_API_TOKEN = prior; });
